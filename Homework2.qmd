...
Add the placebo data from another experiment.

```{r}
# Create a data frame with the results of the experiment for the placebo group.
subjects <- paste("Subject", 1:10, sep = "_")
...
placebo_df <- data.frame(patient = subjects,
                         pre_bp = placebo_pre,
...
```{r}
# Create a list with the treatment and placebo groups.
experiment <- list(treatment = bp_df, 
                   placebo = placebo_df)
